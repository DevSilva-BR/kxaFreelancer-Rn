...
import { getMyFriends } from "../service/api";

const mapsProfissional = (props) => {
    ...
    useEffect(() => {
        ...
        getMyFriends().then(async (res) => {
            res.map(async (friend) => {
                await pesquisaLatLong(friend.address).then(
                    (localizationOfUser) => {
                        const teste = {
                            localicacao: {
                            ...
                            title: friend.nome,
                            description: friend.cartegoriaSelecionada,
                        };
...
export default mapsProfissional;
